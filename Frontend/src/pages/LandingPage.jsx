...
const LandingPage = () => {
  ...
  const getIcon = (iconName, defaultIcon = <MdStar />) => {
    ...
    if (iconName.startsWith('http') || iconName.startsWith('/')) {
      return <img src={iconName} alt="icon" className="w-8 h-8 sm:w-10 sm:h-10 md:w-12 md:h-12 object-contain" />;
    }
    if (iconName.includes('<img') || iconName.includes('src=')) {
      // Extract the src from the HTML string
      const match = iconName.match(/src=["'](.*?)["']/);
      if (match && match[1]) {
        return <img src={match[1]} alt="icon" className="w-8 h-8 sm:w-10 sm:h-10 md:w-12 md:h-12 object-contain" />;
      }
    }
  ...
  // Stats and features are now fetched from backend
        ...
        <section className="relative z-30 -mt-6 sm:-mt-10 md:-mt-14 lg:-mt-16 max-w-7xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8">
          <div className="grid grid-cols-4 gap-2 sm:gap-3 md:gap-4 lg:gap-6">
            {stats.map((stat, idx) => (
              ...
              >
                <div className={`text-lg sm:text-2xl md:text-3xl mb-1 sm:mb-2 md:mb-3 ${stat.iconColor}`}>{getIcon(stat.icon)}</div>
      ...
      {aboutPage && (
        <section id="about" className="py-10 sm:py-16 md:py-24 lg:py-32 bg-slate-50">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className={`grid grid-cols-1 ${aboutPage.imageUrl ? 'md:grid-cols-2' : ''} gap-8 sm:gap-10 md:gap-12 lg:gap-16 items-center`}>
              ...
              {aboutPage.imageUrl && (
                ...
                >
                  <img
                    ...
                    alt={aboutPage.title || "Showroom"}
                    className="relative z-10 rounded-2xl sm:rounded-[2rem] md:rounded-[2.5rem] shadow-[0_20px_50px_-5px_rgba(0,0,0,0.1)] w-full transform -rotate-1 sm:-rotate-2 max-h-[500px] object-cover"
                  />
                </motion.div>
              )}
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 md:gap-8 lg:gap-12">
              {features.map((feat, idx) => (
                ...
                >
                  <div className={`w-12 h-12 sm:w-14 sm:h-14 md:w-16 md:h-16 lg:w-20 lg:h-20 ${feat.colorClass || feat.color} rounded-xl sm:rounded-2xl flex items-center justify-center text-2xl sm:text-3xl md:text-4xl mb-5 sm:mb-6 md:mb-8 group-hover:scale-110 transition-transform shadow-lg`}>
                    {getIcon(feat.icon)}
                  </div>
