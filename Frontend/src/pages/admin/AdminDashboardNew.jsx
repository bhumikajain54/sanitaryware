...
const AdminDashboardNew = () => {
  ...
  const { data: summaryData, loading: summaryLoading, refetch: refetchSummary } = useAdminFetch(fetchSummary, [period]);

  const fetchStats = useCallback(() => adminService.getDashboardStats(period), [period]);
  const { data: statsData, loading: statsLoading, refetch: refetchStats } = useAdminFetch(fetchStats, [period]);
  ...
  const { data: ordersTrendData, loading: ordLoading, refetch: refetchOrd } = useAdminFetch(fetchOrdersTrend, [period]);
  ...
  const { data: usersData, loading: usersLoading, refetch: refetchUsers } = useAdminFetch(fetchUsers, [period]);
    ...
    if (list.length > 0) {
      const transformed = list.map(item => {
        ...
        if (item.date && item.value !== undefined) {
          ...
          // For 7 days use weekday, for longer periods use date for clarity
              ...
              <div className="flex items-start justify-between mb-0.5 sm:mb-3 md:mb-4">
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-0.5 sm:gap-2 mt-0.5 sm:mt-2">
                    {stat.isBadge ? (
                      <span className={`px-1.5 py-0.5 rounded-md text-[8px] font-bold uppercase tracking-widest ${stat.badgeVariant === 'warning' ? 'bg-amber-100 text-amber-700' :
